document.addEventListener('DOMContentLoaded', () => {
    ...
    // Load existing bookings from localStorage (excluding status keys)
    function loadExistingBookings() {
        const out = [];
        for (let i = 0; i < localStorage.length; i++) {
            const k = localStorage.key(i);
            if (/^booking_(?!status_)/.test(k)) {
                try {
    ...
    function computeAvailabilityForDates(checkin, checkout) {
        const inventory = roomInventory || {};
        const bookings = loadExistingBookings();
        ...
        bookings.forEach(b => {
            ...
            if (isOverlap(checkin, checkout, b.checkin, b.checkout)) {
                const key = b.room;
                occupiedByType[key] = (occupiedByType[key] || 0) + 1;
            }
        });
        const availability = {};
        Object.keys(inventory).forEach(type => {
            ...
            availability[type] = { total, occupied: occ, available: Math.max(0, total - occ) };
        });
        return availability;
    }
    ...
    const displayRooms = () => {
        ...
        const availMap = computeAvailabilityForDates(bookingState.checkin, bookingState.checkout);
        roomData.forEach(room => {
            const card = document.createElement('div');
            card.className = 'room-card';
            card.dataset.roomId = room.id;
            const avail = availMap && availMap[room.name] ? availMap[room.name] : { total: 0, available: 0 };
            const availText = roomInventory ? `<p class="room-availability">Available: ${avail.available} / ${avail.total}</p>` : '';
            card.innerHTML = `<img src="${room.img}" alt="${room.name}"><div class="room-details"><h4>${room.name}</h4><p class="room-price">${formatCurrency(room.price)} <span>/ night</span></p>${availText}</div>`;
    ...
    function validateForm() {
        ...
        if (roomInventory && selectedRoom && checkin && checkout) {
            const avail = computeAvailabilityForDates(checkin, checkout)[selectedRoom.name];
            hasCapacity = !!avail && avail.available > 0;
        }
        confirmBtn.disabled = !(checkin && checkout && nights > 0 && selectedRoom && consentGiven && isGuestInfoValid && hasCapacity);
    }
    ...
    function showConfirmation() {
        ...
        if (roomInventory) {
            ...
            if (!avail || avail.available <= 0) {
                alert('Sorry, no availability for this room type on selected dates.');
                return;
            }
        }
